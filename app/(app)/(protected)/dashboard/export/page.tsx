...
useEffect(() => {
  ...
  return (
    <div className="h-[100dvh] bg-gray-50 flex flex-col overflow-hidden md:max-w-md md:mx-auto md:shadow-2xl md:border-x border-gray-200 relative">
      ...
      {/* Header */}
      <div className={`${activeBg} px-4 pt-6 pb-4 shadow-md flex items-center justify-between shrink-0 text-white`}>
        ...
        <button onClick={() => setIsFilterModalOpen(true)} className="relative p-2 bg-white/20 rounded-xl active:bg-white/30 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t.activeFilters}</h3>
            <button onClick={() => setIsFilterModalOpen(true)} className="text-xs font-black text-blue-600">Edit</button>
          </div>
          <p className="text-sm font-bold text-gray-700">
            {getActiveFilterCount() === 0 ? t.noFilters : `${getActiveFilterCount()} rules applied. Ready to export.`}
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-200 flex flex-col gap-4">
          ...
          {/* File Name */}
          <div>
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1 mb-1 block">{t.fileName}</label>
            <input 
              type="text" 
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1 mb-1 block">{t.sortBy}</label>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-xs font-bold text-gray-900 outline-none">
                <option value="serialNumber">{t.sortOptions.serialNumber}</option>
                <option value="fullName">{t.sortOptions.fullName}</option>
                <option value="age">{t.sortOptions.age}</option>
              </select>
            </div>
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1 mb-1 block">{t.order}</label>
              <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value as any)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-xs font-bold text-gray-900 outline-none">
        ...
        <div className="absolute inset-0 z-50 flex flex-col justify-end">
          ...
          <div className="bg-white rounded-t-3xl h-[85vh] flex flex-col relative shadow-2xl animate-in slide-in-from-bottom-full duration-200">
            ...
            <div className="flex justify-between items-center px-5 py-4 border-b border-gray-100 shrink-0">
              <h2 className="text-lg font-black text-gray-900">{t.filterBtn}</h2>
              <button onClick={() => setIsFilterModalOpen(false)} className="bg-gray-100 text-gray-600 w-8 h-8 rounded-full font-bold flex items-center justify-center active:bg-gray-200">✕</button>
            </div>

            {/* Modal Scrollable Filters */}
            <div className="flex-1 overflow-y-auto p-5">
              <FilterPills title={t.villages} available={availableVillages} selected={selectedVillages} setSelected={setSelectedVillages} />
              <FilterPills title={t.castes} available={availableCastes} selected={selectedCastes} setSelected={setSelectedCastes} />
              
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 pl-1">{t.supportLevel}</label>
                  <select value={supportLevel} onChange={(e) => setSupportLevel(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs font-bold outline-none">
                    <option value="">{t.all}</option>
                    <option value="STRONG">Strong</option><option value="NEUTRAL">Neutral</option><option value="WEAK">Weak</option><option value="AGAINST">Against</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 pl-1">{t.votingStatus}</label>
                  <select value={hasVoted} onChange={(e) => setHasVoted(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-xs font-bold outline-none">
                    <option value="">{t.all}</option><option value="true">{t.hasVoted}</option><option value="false">{t.notVoted}</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 pl-1">{t.ageRange}</label>
                <div className="flex gap-2 items-center">
                  <input type="number" placeholder="Min" value={minAge} onChange={(e) => setMinAge(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold outline-none" />
                  <span className="text-gray-400 font-black">-</span>
                  <input type="number" placeholder="Max" value={maxAge} onChange={(e) => setMaxAge(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2 text-sm font-bold outline-none" />
                </div>
              </div>
            </div>
            ...
            <div className="p-4 border-t border-gray-100 shrink-0 sticky bottom-16 pb-safe">
              <button onClick={() => setIsFilterModalOpen(false)} className={`w-full h-14 ${activeBg} text-white rounded-2xl font-black shadow-lg active:scale-95 transition-all`}>
                {t.applyFilters}
              </button>
            </div>
          </div>
